...
namespace SITConnect.Pages
{
    public class ChangePassword : PageModel
    {
        ...
        public IActionResult OnGet()
        {
            ...
            if (passwordLogsByUserId.Any()) CanChangePassword = false;
        ...
        public IActionResult OnPost()
        {
            ...
            var arrayOfPassword = currentUser.Password.Split(";~;");
            var abortPasswordChange = 0;

            // Check the first password history
            if (BCrypt.Net.BCrypt.Verify(NewPassword, arrayOfPassword[0])) abortPasswordChange++;

            // Catch if password no exist
            try
            {
                if (BCrypt.Net.BCrypt.Verify(NewPassword, arrayOfPassword[1])) abortPasswordChange++;
            }
            ...
            try
            {
                if (BCrypt.Net.BCrypt.Verify(NewPassword, arrayOfPassword[2])) abortPasswordChange++;
            }
            catch (IndexOutOfRangeException)
            {
                // Empty
